...
class RobotKinematicsNode(Node):
    """
    Bridge node: Receives 6-DOF targeting from 3D Slicer via OpenIGTLink 
    and solves Inverse Kinematics to drive the RViz robot model.
    """

    # Slicer/OpenIGTLink often truncates node names; accept common variants
    _TRAJECTORY_PREFIX = "Trajectory_ROS_Trans"

    def __init__(self):
        super().__init__("robot_kinematics_node")
        
        # Locate the Xacro file from the robot_control package
        ...
        try:
            ...
            # Replace continuous joints with revolute joints
            urdf_xml = urdf_xml.replace('type="continuous"', 'type="revolute"')
            # Create a temporary file to store the processed URDF
    ...
    def _igtl_to_ros_frame(self, msg: Transform) -> Tuple[np.ndarray, np.ndarray]:
        ...
        1. Scaling: Converts mm (Slicer) to meters (ROS).
        2. Basis Change: Applies a 90-degree Z-axis rotation to align medical RAS 
           coordinates with the robotic World frame.
        """
        # Translation: Swap X/Y and flip sign to match the 90deg rotation
        t = msg.transform.translation
        pos_mm = np.array([t.x, t.y, t.z], dtype=float)
        p_ros = np.array([pos_mm[1], -pos_mm[0], pos_mm[2]], dtype=float) / 1000.0

        # Orientation: Rotate the quaternion rotation matrix into the ROS basis
    ...
    def trajectory_callback(self, msg: Transform):
        """
        Processes incoming surgical targets. 
        Attempts a 6-DOF pose match first, then falls back to 3-DOF translation if 
        orientation is physically unreachable by the arm.
        """
    ...
    def timer_callback(self):
        """Publishes the current joint state to the TF tree."""
        if self.my_chain is None:
            return
        msg = JointState()
